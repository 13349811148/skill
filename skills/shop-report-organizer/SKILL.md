...
- Distinguish an absent column from an empty cell when merging the same order. If the latest source does not contain a column at all, retain that column's value from the older record. If the latest source contains the column but the order's cell is empty, accept the latest empty value. Apply the protected buyer-information rule below after this column-presence check.
