...
2. Detect promotion platform and type by workbook headers, not by folder or file name. A 拼多多 builder may read only a 拼多多 promotion sheet. A 天猫 builder may read only its requested 万相台、新客加速、老客加速 or legacy 品牌新享 sheet. Skip a clearly recognized other-platform sheet; stop when a sheet contains promotion-cost columns but cannot be classified. Recognize legacy 品牌新享 from `业务日期` together with `预估抽佣金额`, and include it under the new-customer acceleration component.
3. Accept promotion date aliases `日期`、`业务日期`、`统计日期`、`统计时间`、`推广日期`、`报表日期`; accept product aliases `商品ID`、`商品id`、`商品Id`、`宝贝ID`、`主体ID`、`商品编号`、`商品编码`、`商品ID（必填）`.
